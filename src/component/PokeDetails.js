                ...
                <Col xs={12} sm={7}>
                    ...
                    <Container className={"moves-list poke-" + types[0].type.name}>
                        <CardColumns className={"card-column-style"}>
                            {moves? (moves.map((m => <Card className={"card-style poke-" + types[0].type.name + "-light"}> <Card.Body className="text-capitalize text-adjust font-weight-bold px-2"> {(m.move.name).replace('-', ' ')} </Card.Body> </Card>))): null}
                        </CardColumns>
                    </Container>
                </Col>
            </Row>
